...
Measured: on a LoCoMo conversation spanning 166 days, strict session recall@5 goes
from 0.263 with `--timeline now` (decay neutralized) to **0.000** with
`--timeline anchored` (real relative gaps preserved). Same corpus, same ranker, same
embedder — the only difference is that the decay term is allowed to act.
