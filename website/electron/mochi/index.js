...
function startMochiWatcher() {
  ...
  ipcMain.handle("mochi-shortcuts:apply", (_e, accelerators) => {
    ...
    try {
      // PERSIST before binding. The shell's store is the source of truth for the
      // accelerators now, and the reconcile loop's drift check reads it — so a
      // rebind that bound but did not persist would be silently undone by the
      // very next tick. `byUser` records the intent, so a migration that lands
      // later cannot import the stale gateway copy over this rebind.
      setShortcutsIn(machineStore, desired, { byUser: true });
      return applyMochiShortcuts(shortcutsOf(machineStore)) || {};
    } catch (err) {
  ...
  ipcMain.handle("mochi-instances:set", async (_e, instanceId) => {
    try {
      ...
      await reconcileMochiAfterCurrent();
      return { ok: true, petInstance: saved };
    } catch (err) {
