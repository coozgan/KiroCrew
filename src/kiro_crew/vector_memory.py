...
class VectorMemoryStore:
    ...
    def write_lesson(
        ...
        # PREFLIGHT the final value BEFORE the dedup loop below, which DELETES
        # superseded rows. The value is only validated by set_semantic at the very
        # end, so a value this store would reject (e.g. an injection-pattern
        # ``negative``) previously cost the caller its existing lesson: the dedup
        # scan deleted the old row, then set_semantic refused the replacement and
        # the route still returned HTTP 200 with no lesson stored. Validating here
        # makes the whole call a no-op when the replacement cannot land.
        slug = hashlib.md5(rule.encode(), usedforsecurity=False).hexdigest()[:12]
        key = f"lesson.{slug}"
        value = rule if not negative else f"{rule}{LESSON_NEGATIVE_SEP}{negative}"
