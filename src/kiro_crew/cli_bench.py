...
def _load_report(path: str, label: str) -> dict:
    ...
    try:
        ...
        print(f"error: {label} report: {exc}")
        raise _BenchError(1) from exc
    except FileNotFoundError:
        print(f"error: {label} report not found: {path}")
        raise _BenchError(1) from None
    except OSError as exc:
        print(f"error: cannot read {label} report {path}: {exc}")
        raise _BenchError(1) from exc
    ...
    try:
        ...
        print(
            f"error: {label} report {path} is not valid JSON ({exc}).\n"
            "A report truncated by an interrupted run is the usual cause -- "
            "re-run 'kirocrew bench retrieval' to regenerate it."
        )
        raise _BenchError(1) from exc

    if not isinstance(loaded, dict):
        print(f"error: {label} report {path} is not a report object")
        raise _BenchError(1)
    return loaded
...
def _retrieval(args: argparse.Namespace) -> int:
    ...
    except RetrievalNotMeasurable as exc:
