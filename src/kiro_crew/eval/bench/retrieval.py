...
def _metric_block(results: Sequence[QueryRetrieval], k_values: Sequence[int], level: str
                  ) -> dict[str, float]:
    out: dict[str, float] = {}
    if not results:
        return out
    for k in k_values:
        for name, fn in (
            ...
            vals = [
                fn(
                    r.retrieved_session_ids if level == "session" else r.retrieved_turn_ids,
                    r.gold_session_ids if level == "session" else r.gold_turn_ids,
                    k,
                )
                for r in results
                if (r.gold_session_ids if level == "session" else r.gold_turn_ids)
            ]
            if vals:
...
def aggregate(
    ...
    *,
    ...
    agg.session = _metric_block(results, k_values, "session")
    agg.turn = _metric_block(results, k_values, "turn")
    ...
    agg.by_category = {
        cat: _metric_block(rs, k_values, "session") for cat, rs in sorted(buckets.items())
    }
    return agg
