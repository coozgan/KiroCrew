...
async def _mutate_instance(request: web.Request, op: str) -> web.Response:
    ...
    def _work() -> dict:
        ...
        # The instance id drives the registry cleanup below. The dashboard sends
        # it, but it is an OPTIONAL query param, so a caller that omits it would
        # silently skip the unregister and leave a deleted crew listed forever.
        # Resolve it from the stack itself — and BEFORE the delete, because the
        # outputs are unreadable once the stack is gone.
        iid = instance_id
        if not iid:
            try:
                iid = str(ec2.describe(tag, profile, region).get("instance_id") or "")
            except Exception as e:
                # Deliberately broad: this lookup is best-effort ENRICHMENT, and the
                # fallback (empty id -> skip the unregister) is exactly the behaviour
                # before it existed. Letting anything escape here would turn a
                # perfectly good delete into an error and strand the user with a crew
                # they cannot remove — strictly worse than a stale registry row.
                # AWSError is not enough: describe shells out, so an exec/sandbox
