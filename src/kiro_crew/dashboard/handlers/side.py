...
def _requeue_unconsumed_side_steers(state: DashboardState, slot, run_id: str) -> None:
...
async def _run_side_turn(
    ...
    *,
    ...
    def _on_steer_consumed(snapshot: str) -> None:
        ...
        Whatever stays pending never reached a generation, and the finally
        below turns it back into a visible queue card.
        """
        side = slot._side
        if side is None or not side.pending_steers:
            return
        side.pending_steers[:] = settle_consumed_steers(side.pending_steers, snapshot)
    ...
    try:
        ...
        _requeue_unconsumed_side_steers(state, slot, run_id)
...
async def api_side_turn(request: web.Request) -> web.Response:
    ...
    if slot._side.last_run_id and not slot._side.is_complete:
        ...
        if body.get("steer") and await _try_side_steer(state, slot, question):
            side_after = slot._side
            if side_after is not side_before or not side_after.open:
                # The sidecar the steer was aimed at is gone; there is nothing to
                # attach the text to, and the replacement never asked for it.
                return web.json_response(
                    {
                        "error": "side conversation is not open",
                        "code": "side_not_open",
