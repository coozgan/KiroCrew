...
def remove_playwright_servers(config: dict) -> bool:
    """Remove EVERY KiroCrew Playwright-proxy server from ``config`` and scrub its
...
def _owned_agent_config_files() -> list[Path]:
    """The EXACT KiroCrew-generated agent-config files that exist on disk.
    ...
    filenames is what keeps a rewrite from touching configs KiroCrew did not
    author.
    """
    files: list[Path] = []
...
def _apply_to_owned_agent_files(transform: "Callable[[dict], bool]") -> None:
    ...
    for path in _owned_agent_config_files():
        ...
        try:
            ...
            # grant/autoApprove must not survive a sweep of a KiroCrew-owned agent
            # config. No-op on an ungoverned host.
            try:
...
def _remove_playwright_from_agent_files() -> None:
    """Sweep KiroCrew-generated agent configs, removing the Playwright proxy and
    its tool references. Runs when Browser Mode is disabled so the ``browser_*``
