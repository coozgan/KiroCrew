...
Imports are deliberately lazy (inside the function): channel modules pull in
their vendor clients, and callers such as ``_channel_members()`` run this off
the event loop in an executor precisely so those imports never block the loop.
