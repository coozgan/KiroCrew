...
class TestReplyResumes:
    def test_resume_helper_lifts_the_flag(self, session_map):
        ...
        _resume_if_paused(session_map, "dashboard:chat-1", SLACK_TS)

        assert session_map.is_slack_paused("dashboard:chat-1") is False

    def test_resume_is_a_noop_on_a_live_link(self, session_map):
        """Guarded on the read so the blocking save runs only on the transition."""
        from kiro_crew.slack.handler import _resume_if_paused
    ...
    def test_resume_swallows_a_session_manager_without_the_accessor(self, session_map):
        from kiro_crew.slack.handler import _resume_if_paused

        _resume_if_paused(object(), "dashboard:chat-1", SLACK_TS)
