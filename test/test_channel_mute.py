...
class TestAReplyLiftsTheMute:
    ...
    def test_a_reply_to_a_muted_binding_lifts_the_mute(self, session_map):
    ...
    def test_a_stubbed_session_manager_does_not_fake_a_resume(self):
        """`is True`, not truthiness: a MagicMock is truthy and would "resume"."""
        stub = MagicMock()
        lifted = self._resume(stub).resume_if_muted(self.CHANNEL, self.KEY)
